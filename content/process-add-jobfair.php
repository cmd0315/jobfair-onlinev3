...
//INSERT JOB FAIR
$selectJFairQuery = "SELECT id FROM job_fair WHERE code='$jobFairCode' AND status='0'";
$selectJFair = mysql_query($selectJFairQuery) or die(mysql_error());
$jFairCount = mysql_num_rows($selectJFair);

if(jFairCount == 0){
	$addJobFairQuery = "INSERT INTO job_fair(code, title, establishment_name, street, location_id, date_scheduled, duration, start_time, end_time, num_vacancies, website_link, first_name, middle_name, last_name, mobile, email, landline, date_added) VALUES('$jobFairCode', '$title', '$establishmentName', '$street', '$location', '$jobFairDate', '$numDays', '$timeStart', '$timeEnd', '$numVacancies', '$websiteLink', '$firstName', '$middleName', '$lastName', '$mobile', '$email', '$landline', '$currDateTime')";
		$response = 1;

	$addJobFair = mysql_query($addJobFairQuery) or die(mysql_error());

	//add log activity
	$addLogQuery = "INSERT INTO activity_logs(date_made, username, action, reference_object) VALUES('$currDateTime', '$username', 'ADD JOB FAIR', '$jobFairCode')";
