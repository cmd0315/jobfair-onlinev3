...
//add log activity
$addLogQuery = "INSERT INTO activity_logs(date_made, username, action, reference_object) VALUES('$currDateTime', '$sessionUsername', 'EXPORT JOB APPLICATIONS', 'ALL JOB POSTS')";
...
$objPHPExcel->getProperties()->setCreator("JobFair-Online.Net 2013")
							 ...
							 ->setDescription("Job Application Sheet")
...
$objPHPExcel->setActiveSheetIndex(1)
			...
			->setCellValue('H2', $jobApplicationNum . $records2)
...
$objPHPExcel->setActiveSheetIndex(1)->mergeCells('F4:P4');
...
foreach($jobPosts as $jobPostCode){
	...
	$jobApplicationNum = mysql_num_rows($getJobApplication);
	$records2 = "";
	if($jobApplicationNum > 1){
		$records2 = " Records";
	}
	else{
		$records2 = " Record";
...
while($jobApplication = mysql_fetch_assoc($getJobApplication)){
		...
		$jobPostCode = $jobApplication['job_code'];
		//$jobPos = getJobPost($jobPostCode, "job-pos");
		$jobPos = getJobPost($jobPostCode, "job-pos-name");
...
$objPHPExcel->getActiveSheet()->getStyle('G2:N2')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_RIGHT);
...
header('Content-Disposition: attachment;filename="Job Application Records(JOBFAIR-ONLINE.NET).xls"');
