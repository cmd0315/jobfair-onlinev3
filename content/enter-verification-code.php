	...
	<div id="container-fluid">
								...
								<div class="row-fluid">
									<div class="control-group">
										...
										<div class="controls controls-row">
											  ...
											  <p class="help-block" id="noCodeResult" style="display:none; text-align:center; color:red;"><strong>Error!</strong>  Invalid code. Please try again.</p>
										</div>
									</div>
								</div>
							</div>
		...
		<div class="row-fluid">
			<div class="span2 offset7">
				<button type="button" class="btn btn-primary span12" id="verifyAccount" name="verifyAccount" onclick="verifyCode()" style="font-weight:bold;">VERIFY ACCOUNT</button>			
			</div>
		</div>
	</div>
	<div class="push"></div>
	...
	function verifyCode(){
			...
			hr.onreadystatechange = function() {
				if(hr.readyState == 4 && hr.status == 200) {
					...
					else if (return_data == "already verified") {
						document.getElementById("noCodeResult").style.display = "block";
					}
					else {
						window.location.href = "process-reverify-account.php";
					}
				}
			}
			hr.send(vars);
